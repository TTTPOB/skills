...
| Tables and equations | Use a native table for real row-and-column data, and a native equation when structured math typesetting is needed. Do not fake a table with textboxes, or treat raw underscore input as a finished equation. Calibrate the linear input syntax with a representative test equation and confirm the typeset result rather than judging by the UI's input-mode setting. |
| Page numbers | Use PowerPoint's native slide number, never a hand-typed number per slide. Confirm the numbers actually appear before removing any manual ones, so the deck does not end up with none or with two. See [MCP Recipes](references/mcp-recipes.md). |
